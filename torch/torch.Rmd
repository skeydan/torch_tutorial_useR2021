...
The model is a convolutional neural network that filters and downsizes its input, finally arriving at a class prediction. There are ten types of digits (0-9), and for each digit, it will output a score for each of the ten possible labels. In the definition of the model, note the parameter `num_classes`. In this example, there is no real need to parameterize the number of classes the model can work with; however, we'd like to show how with `liz`, you can keep the model definition flexible and pass the desired configuration at training time.
